...
# Load and clean data
data <-  read_delim("DrugsAlcohol.txt")  %>% 
...
# Assume the dose is drawn from a uniform distribution
dat <- dat %>%
  mutate(dose = runif(n(), 0, 1))  # Define dose uniformly across counties

# Calculate the treatment effect (DiD) based on the dose
dat_c <- dat %>%
  group_by(CountyCode) %>%
  mutate(
    # Define the ATE as a non-linear function of the dose (e.g., dose^2)
    true_treatment_effect = if_else(treated == 1 & post_period == 1, dose^2, 0),
    ate = true_treatment_effect,
    
    # Remove deaths probabilistically based on the ATE
    deaths_remove = if_else(post_period == 0, 0, sum(rbinom(n = Deaths, size = 1, prob = ate))),
    deaths_sim = Deaths - deaths_remove,
    
    # Calculate the new death rate after accounting for the treatment effect
    true_ate_prob = deaths_remove / Deaths,
    deathrate_sim = (deaths_sim / Population) * 100000,
    
    # Calculate the change in death rate
    true_death_rate = (Deaths / Population) * 100000,
    true_att_calc = deathrate_sim - true_death_rate
  ) %>%
  ungroup() %>%
  mutate(
    sd_deathrate = sd(true_death_rate, na.rm = TRUE),
    pre_treat_deathrate = mean(true_death_rate[treated == 1 & post_period == 0], na.rm = TRUE)
  )
