...
dat <- read.csv("/home/oddish3/Documents/uni/master-dissertation/code-cont/data.csv")

# Filter out rows where dose is 0
data <- dat[dat$dose != 0, ]

# Extract x and y and sort them in increasing order
sorted_indices <- order(data$dose)
x <- data$dose[sorted_indices] # dose level
y <- data$dy[sorted_indices]
...
binarised <- feols(dy ~ binary, data=dat)
binarised
feols(dy ~ dose, data=dat) # twfe
...
treated_hospitals <- data[data$dose > 0, ]
...
E_Y_D0 <- mean(dat$dy[dat$dose == 0]) # sample mean of outcome for control group (second term)
positive_doses <- dat$dose[dat$dose > 0]
u_hat <- data$dy - E_Y_D0 - hhat[findInterval(positive_doses, Xx_sub)]
...
W <- data
# Apply the function
eta_values <- eta_acr_o(W[W$dose > 0, ])
variance_ACR <- mean(eta_values^2)
se_ACR <- sqrt(variance_ACR / length(eta_values))

# Calculate ACR^o estimate
ACR_estimate <- mean(dhat[findInterval(W$dose[W$dose > 0], Xx_sub)])
print(paste("ACR^o estimate:", ACR_estimate))
print(paste("Standard Error:", se_ACR))

# Calculate the t-statistic and p-value
n_treated <- sum(W$dose > 0)
k <- ncol(spline_dosage_SR)  # number of basis functions
df <- n_treated - k
